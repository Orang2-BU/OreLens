...
class CorrelationTests(TestCase):
    ...
    def test_screen_driver_accepts_twelve_transformed_aligned_periods(self):
        ...
        self.assertEqual(len(result['validation']['rolling_correlations']), 1)
        self.assertEqual(result['validation']['train_correlation'], 1.0)
        self.assertEqual(result['validation']['test_correlation'], 1.0)
