            ...
            <table className="comparison-table">
              ...
              <tbody>
                {data.rows.map(({ company, exposure, intelligence }) => {
                  ...
                  return (
                    <tr key={company.id}>
                      <th scope="row"><Link className="company-link" href={`/companies/${company.id}?commodity=${data.commodity.code}`}><strong>{company.ticker}</strong><span>{company.name}</span></Link><small>{formatMarketCap(company.market_cap, company.currency)} · {company.exchange}</small></th>
